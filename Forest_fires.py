...
def one_hot_encode(df, column):
    ...
    encoded df pd.DataFrame(encoded_array.toarray(), columns=ohe.get_feature_names([column]))
...
def app():
    ...
    if uploaded file is not None:
        ...
        df imputed impute_null_values (df, impute_method)
        
        # Display a dropdown menu to select the encoding technique
        encoding method = st.selectbox("select an encoding technique", options=['None', 'Label Encoding', 'One-Hot Encoding'])
        
        #Perform the selected encoding technique
        if encoding method == 'Label Encoding':
        ...
        elif encoding method One-Hot Encoding': 
